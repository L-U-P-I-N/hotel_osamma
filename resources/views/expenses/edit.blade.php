        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="currency" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-blue-400">
                    <option value="YER" {{ old('currency',$expense->currency)=='YER'?'selected':'' }}>ريال يمني</option>
                    <option value="SAR" {{ old('currency',$expense->currency)=='SAR'?'selected':'' }}>ريال سعودي</option>
                    <option value="USD" {{ old('currency',$expense->currency)=='USD'?'selected':'' }}>دولار أمريكي</option>
                </select>
            </div>
        </div>
