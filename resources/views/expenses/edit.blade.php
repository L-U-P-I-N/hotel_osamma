    ...
    <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-4">
        ...
        <div>
            ...
            <select name="employee_id" id="employee_select"
                ...
                <option value="{{ $emp->id }}" data-name="{{ $emp->name }}" {{ old('employee_id', $expense->employee_id) == $emp->id ? 'selected' : '' }}>
                    {{ $emp->name }} — {{ $emp->position }}
                </option>
                @endforeach
            </select>
            <p id="employee_hint" class="{{ old('employee_id', $expense->employee_id) ? '' : 'hidden' }} text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2 mt-1.5">
        ...
        <div>
            ...
            <input type="text" name="recipient_name" id="recipient_field" value="{{ old('recipient_name', $expense->recipient_name) }}"
                   placeholder="اسم الشخص الذي صُرف له المبلغ" required
