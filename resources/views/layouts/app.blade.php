...
@can('checkin.view')
            ...
            @can('reports.view')
            @php
                // الصندوق العام وكشف الحسابات خرجا من قائمة التقارير إلى قسم
                // مستقل، فلا يُفتح قسم التقارير ولا يُظلَّل عند فتح أيٍّ منهما.
                $inReports = request()->routeIs('reports.*')
                    && !request()->routeIs('reports.generalSafe*')
                    && !request()->routeIs('reports.accountSearch');
            @endphp
            <div class="nav-section-label mt-2">تقارير</div>
            <div x-data="{ open: {{ $inReports ? 'true' : 'false' }} }">
                <button @click="open=!open"
                        class="nav-link w-full justify-between {{ $inReports ? 'active' : '' }}">
            ...
            <!-- النظام -->
            @canany(['users.manage', 'pricing.manage'])
            <div class="nav-section-label mt-2">النظام</div>
            @endcanany
            @can('reports.view')
            <a href="{{ route('coa.index') }}"
               class="nav-link {{ request()->routeIs('coa.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('pricing.index') }}"
               class="nav-link {{ request()->routeIs('pricing.*') ? 'active' : '' }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5a1.99 1.99 0 011.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                إعدادات التسعير
            </a>
            @endcan
            @can('users.manage')
            ...
            <div x-data="{ open: {{ request()->routeIs('employees.*','salaries.*','attendance.*','leaves.*') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" x-cloak class="mt-0.5 mb-1 mr-5 space-y-0.5 border-r border-white/10 pr-2">
                    ...
                    <a href="{{ route('employees.statements') }}" class="nav-link text-xs {{ request()->routeIs('employees.statements') ? 'active' : '' }}">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        كشف حساب الموظفين
                    </a>
