...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <div class="text-2xl font-bold text-gray-800 mt-1">{{ number_format($m->total, 0) }}</div>
        <div class="text-xs text-gray-400 mt-0.5">ريال يمني</div>
    </div>
    @endforeach
    <div class="rounded-xl p-5 border" style="background:#e8f0f7; border-color:#9fbedd;">
        ...
        <div class="text-2xl font-bold mt-1" style="color:#0F4C75;">{{ number_format($totalRevenue, 0) }}</div>
        <div class="text-xs mt-0.5" style="color:#5b90c5;">ريال يمني</div>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
    <h3 class="font-semibold text-gray-700 mb-4">الإيرادات حسب نوع الغرفة</h3>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100">
    <div class="px-6 py-4 border-b border-gray-100"><h3 class="font-semibold text-gray-700">تفصيل الإيرادات</h3></div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 font-bold text-green-700">{{ number_format($r->total, 0) }} ر.ي</td>
                    <td class="px-4 py-3 text-gray-500">
                        {{ $totalRevenue > 0 ? round(($r->total / $totalRevenue) * 100) : 0 }}%
                    </td>
                </tr>
                @empty
                <tr><td colspan="3" class="px-4 py-6 text-center text-gray-400">لا توجد بيانات</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
