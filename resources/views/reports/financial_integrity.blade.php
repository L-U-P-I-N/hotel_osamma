            ...
            @else
                ...
                <div class="overflow-x-auto"><table class="w-full text-sm">
                    <thead class="bg-gray-50"><tr>
                        ...
                        <th class="px-4 py-2 text-right text-xs font-medium text-gray-500">مصدر التمويل</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-gray-500">الموظف المرتبط</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-gray-500">مَن سجّله</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 text-xs text-gray-700">{{ $w->handed_by_name ?: '—' }}</td>
                            ...
                            <td class="px-4 py-2 text-xs text-gray-600">
                                ...
                                <div class="text-[11px] text-gray-400">مولَّدة من مصروف #{{ $w->expense_id }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-2 text-xs text-gray-600">{{ $w->funding_source_label }}</td>
                            <td class="px-4 py-2 text-xs text-gray-600">{{ $w->employee?->name ?? '—' }}</td>
                            {{-- مَن سجّل السحب فعلياً: يأتي من المصروف المرتبط إن كانت مولَّدة منه --}}
                            <td class="px-4 py-2 text-xs text-gray-600">{{ $w->expense?->paidBy?->name ?? '—' }}</td>
                            <td class="px-4 py-2 text-xs">
                                @if($suggested)
                                <span class="text-gray-700">{{ $suggested->user?->name ?? 'وردية #' . $suggested->id }}</span>
                                <div class="text-[11px] text-gray-400">{{ $suggested->shift_date?->format('d/m/Y') }}</div>
                                @else
                                <span class="text-gray-300">لا توجد وردية في ذلك التاريخ</span>
                                @endif
                            </td>
