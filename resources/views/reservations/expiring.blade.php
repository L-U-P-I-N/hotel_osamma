...
@push('scripts')
<script>
    // بحث تلقائي بعد توقف الكتابة (لا فوري مع كل حرف) — يمنح وقتاً كافياً
    // لموظف يكتب ببطء دون أن يقاطعه البحث في منتصف الكلمة.
    let __searchDebounceTimer = null;
    function debounceSearch(input) {
        clearTimeout(__searchDebounceTimer);
        __searchDebounceTimer = setTimeout(() => input.form.submit(), 900);
    }

    @if(request('search'))
    // الصفحة أُعيد تحميلها بسبب بحث فعّال — نعيد التركيز فوراً لحقل البحث
    // (والمؤشر في آخر النص) حتى يكمل الموظف الكتابة دون النقر على الحقل من جديد.
