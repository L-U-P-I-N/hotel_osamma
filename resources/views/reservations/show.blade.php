...
<div class="flex-shrink-0 hero-gradient rounded-2xl shadow-lg px-4 py-3">
    <div class="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div class="flex items-center gap-2.5 flex-wrap min-w-0">
            ...
            {{-- تظهر دائماً (حتى بلا مرافقين) — من هنا يمكن إضافة مرافق مباشرة دون
                 الحاجة لفتح تعديل الحجز كاملاً --}}
        ...
        <div class="flex items-center gap-1.5 flex-wrap justify-end">
            ...
            <div class="relative" x-data="{ moreOpen: false }">
                ...
                <div x-show="moreOpen" x-cloak x-transition
                    ...
                    @endif
                    @endcan
                    {{-- إعادة إظهار تنبيه إعادة الاحتساب — للمدير فقط، ويظهر فقط إن كان
                         التنبيه مُخفى لهذا الحجز. الإخفاء يُجرى من كارت التفاصيل، أما
                         التراجع عنه فمقصور على المدير حتى لا يُعاد إظهاره عن طريق الخطأ. --}}
                    @if(auth()->user()->isAdmin() && $reservation->recompute_dismissed && in_array($reservation->status, ['checked_in', 'checked_out']))
                    <div class="border-t border-gray-100 my-1"></div>
                    <form method="POST" action="{{ route('reservations.toggleRecomputeDismiss', $reservation) }}">
                        @csrf
                        <button type="submit" class="w-full flex items-center gap-2.5 px-4 py-2 text-blue-700 hover:bg-blue-50 transition">
                            <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            إعادة إظهار تنبيه إعادة الاحتساب
                        </button>
                    </form>
                    @endif
...
<div class="flex-shrink-0 grid grid-cols-2 md:grid-cols-4 gap-3">
    ...
    <div class="col-span-2 rounded-2xl shadow-sm border p-4 flex flex-col justify-center {{ $reservation->balance > 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200' }}">
        <div class="flex items-center gap-2 mb-1">
            <svg class="w-5 h-5 {{ $reservation->balance > 0 ? 'text-red-500' : 'text-emerald-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
            <p class="text-sm font-bold {{ $reservation->balance > 0 ? 'text-red-600' : 'text-emerald-700' }}">
                {{ $reservation->balance > 0 ? 'المطلوب تحصيله من النزيل' : 'الحساب مكتمل ✓ لا يوجد متبقٍّ' }}
            </p>
        </div>
        <p class="text-4xl font-black {{ $reservation->balance > 0 ? 'text-red-600' : 'text-emerald-600' }} leading-none">
            {{ number_format(abs($reservation->balance), 0) }} <span class="text-lg font-bold text-gray-400">{{ $reservation->currency_symbol }}</span>
        </p>
        @if($reservation->total_amount > 0)
        <div class="w-full bg-white/70 rounded-full h-2 mt-3 overflow-hidden">
            <div class="paid-bar h-2 rounded-full" style="width: {{ $paidPercent }}%"></div>
        </div>
        <p class="text-xs text-gray-500 mt-1">تم دفع {{ $paidPercent }}% من الإجمالي</p>
        @endif
    </div>
    {{-- الإجمالي --}}
    <div class="rounded-2xl shadow-sm border border-gray-100 bg-white p-4 flex flex-col justify-center">
        <p class="text-xs text-gray-400 font-semibold mb-1.5">الإجمالي المطلوب</p>
        <p class="text-2xl font-black text-gray-900 leading-none">{{ number_format($reservation->total_amount, 0) }}<span class="text-sm font-bold text-gray-400"> {{ $reservation->currency_symbol }}</span></p>
    </div>
    {{-- المدفوع --}}
    <div class="rounded-2xl shadow-sm border border-gray-100 bg-white p-4 flex flex-col justify-center">
        <p class="text-xs text-gray-400 font-semibold mb-1.5">المبلغ المدفوع</p>
        <p class="text-2xl font-black text-emerald-600 leading-none">{{ number_format($reservation->paid_amount, 0) }}<span class="text-sm font-bold text-gray-400"> {{ $reservation->currency_symbol }}</span></p>
    </div>
</div>
                ...
                <div class="px-5 py-2.5 flex items-center justify-between text-sm">
                    ...
                    <div class="flex items-center gap-2 ml-3">
                        ...
                        <button type="button"
                                ...
                                class="p-1 rounded-lg text-blue-600 hover:bg-blue-50 transition" title="تعديل الرسم">
                        ...
                        <form method="POST" action="{{ route('reservations.deleteCharge', $charge) }}"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-1 rounded-lg text-red-600 hover:bg-red-50 transition" title="حذف الرسم">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                            </button>
                        </form>
                        @endif
            ...
            <div class="bg-gray-50 rounded-xl p-3 space-y-2">
                ...
                <div class="flex items-center justify-between text-xs">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {{-- حذف الدفعة: يظهر فقط لمن استلمها بنفسه أو للمدير، ويُمنَع إن
                             كانت تخصّ وردية مقفلة (يُتحقّق مجدداً في الخادم). الحذف
                             يُزيلها من رصيد الحجز وحالة الدفع ومجاميع الوردية معاً. --}}
                        @php $payShiftClosed = $p->shift && $p->shift->is_closed; @endphp
                        @if((auth()->user()->isAdmin() || $p->received_by === auth()->id()) && !$payShiftClosed)
                        <form method="POST" action="{{ route('payments.destroy', $p) }}" class="inline"
                              onsubmit="return confirm('حذف هذه الدفعة ({{ number_format($p->amount, 0) }} {{ $reservation->currency_symbol }})؟ ستُزال من رصيد الحجز ومن مجاميع الوردية. لا يمكن التراجع.')">
                            @csrf @method('DELETE')
                            <button type="submit" class="inline-flex items-center gap-1 text-gray-400 hover:text-red-600 font-semibold hover:underline">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                حذف
                            </button>
                        </form>
                        @elseif($payShiftClosed && (auth()->user()->isAdmin() || $p->received_by === auth()->id()))
                        <span class="inline-flex items-center gap-1 text-gray-300" title="لا يمكن حذف دفعة تخصّ وردية مقفلة — افتح الوردية أولاً">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                            مقفلة
                        </span>
                        @endif
...
{{-- ===== EDIT CHARGE MODAL (متاح للمقيم والمغادر) ===== --}}
@can('payments.create')
<div id="editChargeModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto"
     onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto my-auto" onclick="event.stopPropagation()">
        <div class="hero-gradient px-6 py-5 rounded-t-2xl flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h3 class="font-bold text-white text-lg">تعديل الرسم الإضافي</h3>
            </div>
            <button type="button" onclick="document.getElementById('editChargeModal').classList.add('hidden')"
                    class="w-8 h-8 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/20 rounded-lg transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <form id="editChargeForm" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            <div class="flex items-start gap-2 rounded-xl bg-emerald-50 border border-emerald-200 p-3 text-xs text-emerald-800">
                <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>سيُعدَّل المبلغ في حساب النزيل تلقائياً بفارق المبلغ.</span>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">نوع الرسم <span class="text-red-500">*</span></label>
                <select id="editChargeType" name="charge_type" required
                        class="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-400 outline-none bg-white">
                    <option value="بقالة">بقالة</option>
                    <option value="مأكولات">مأكولات ومشروبات</option>
                    <option value="خدمة غرفة">خدمة غرفة</option>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">الوصف <span class="text-gray-400 font-normal">(اختياري)</span></label>
                <input id="editChargeDescription" type="text" name="description" maxlength="255"
                       class="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-400 outline-none">
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">المبلغ (ر.ي) <span class="text-red-500">*</span></label>
                <input id="editChargeAmount" type="number" name="amount" min="0.01" step="0.01" required
                       class="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-400 outline-none">
            </div>
